  ...
  const searchsinger = async () => {
    try {
      const response = await axios.get(
        `${process.env.BASE_URL}/api/client/singer/searchsinger?search=${searchQuery}`
      );
  ...
  const updateToIsPopular = async (id) => {
    try {
      const response = await axios.post(
        `${process.env.BASE_URL}/api/backend/singer/updatetoispopular?id=${id}`
      );
              ...
              singerList.map((singer) => (
                ...
                >
                  ...
                  <td className="py-4 px-6">
                    <button
                      onClick={() => updateToIsPopular(singer._id)}
                      className="font-medium text-blue-600 dark:text-blue-500 hover:underline"
                    >
                      {(() => {
                        switch (singer.isPopular) {
                          case 0:
                            return "False";
                          case 1:
                            return "True";
